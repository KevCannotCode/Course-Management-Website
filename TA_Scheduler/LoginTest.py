...
class login_test(TestCase):
    ...
    def test_good_login(self):
        ...
        self.assertEqual("/homepage/", response.url,"Log in as admin with correct credentials admin failed."
                                                    "Expected redirect URL to be /homepage/")
        #Good login with standard user
        response = self.client.post("/", {"name": self.standardUser.name, "password": self.standardUser.password})
        #Check redirect: Should it redirect to a homepge when the user logs in?
        self.assertEqual("/homepage/", response.url,"Log in a standard user with correct credentials admin failed."
                                                    "Expected redirect URL to be /homepage/")
